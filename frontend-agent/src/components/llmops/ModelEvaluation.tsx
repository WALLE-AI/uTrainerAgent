...
const BenchmarkRadar: React.FC<{ data: any[] }> = ({ data }) => {
    const radarData = [
    ...
    return (
        <div className="bg-white/50 backdrop-blur-xl border border-slate-100 rounded-[32px] p-6 shadow-sm h-[400px]">
            <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">能力雷达图 / Capability Radar</h3>
...
export const ModelEvaluation: React.FC = () => {
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 text-left">
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    {[
                        ...
                        { name: '医学 (Medical)', label: '医学', icon: ICONS.briefcase },
                        { name: '法律 (Legal)', label: '法律', icon: ICONS.briefcase },
                        { name: '金融 (Finance)', label: '金融', icon: ICONS.briefcase },
                        ...
                        >
                            <div className={`w-12 h-12 rounded-2xl mb-4 flex items-center justify-center transition-all duration-500 ${category === cat.name ? 'bg-white/20' : 'bg-slate-50 group-hover:bg-indigo-100 group-hover:text-indigo-600 group-hover:rotate-6'
                                }`}>
                ...
                <div className="bg-white/70 backdrop-blur-xl border border-slate-100 rounded-[32px] overflow-hidden shadow-sm">
                    <div className="p-6 border-b border-slate-50 flex justify-between items-center">
                        <h3 className="text-xs font-black text-slate-900 uppercase tracking-[0.2em]">{category} 性能排行榜</h3>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-slate-50/50">
                                <tr>
                                    <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">排名 Rank</th>
                                    <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">模型 Model</th>
                                    {isGeneral ? (
                                        <>
                                            <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">MMLU</th>
                                            <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">C-Eval</th>
                                            <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">GSM8K</th>
                                            <th className="px-6 py-4 text-[10px] font-black text-indigo-600 uppercase tracking-widest text-center">综合 Score</th>
                                        </>
                                    ) : (
                                        <>
                                            <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">准度 Accuracy</th>
                                            <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">推理 Reasoning</th>
                                            <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">知识 Knowledge</th>
                                    ...
                                    <tr key={item.name} className="hover:bg-slate-50/50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black ${idx === 0 ? 'bg-yellow-400 text-white shadow-lg shadow-yellow-100' :
                                                idx === 1 ? 'bg-slate-300 text-white shadow-lg shadow-slate-100' :
                                        ...
                                        {isGeneral ? (
                                            <>
                                                <td className="px-6 py-4 text-center font-mono text-xs text-slate-600">{item.mmlu}</td>
                                                <td className="px-6 py-4 text-center font-mono text-xs text-slate-600">{item.ceval}</td>
                                                <td className="px-6 py-4 text-center font-mono text-xs text-slate-600">{item.gsm8k}</td>
                                            ...
                                            <>
                                                <td className="px-6 py-4 text-center font-mono text-xs text-slate-600">{item.accuracy}</td>
                                                <td className="px-6 py-4 text-center font-mono text-xs text-slate-600">{item.reasoning}</td>
                                                <td className="px-6 py-4 text-center font-mono text-xs text-slate-600">{item.knowledge}</td>
                                                <td className="px-6 py-4 text-center">
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 text-left">
            <div className="bg-white/70 backdrop-blur-xl border border-slate-100 rounded-[32px] overflow-hidden shadow-sm">
                <table className="w-full text-left">
                    <thead className="bg-slate-50/50">
                        <tr>
                            <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">任务 ID</th>
                            <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">任务名称</th>
                            <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">模型 Model</th>
                            <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">状态 Status</th>
                            <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">日期 Date</th>
                            <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">用时</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                        {evaluationTasks.map(task => (
                            <tr key={task.id} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-sm font-black text-slate-900">{task.name}</td>
                                <td className="px-6 py-4 text-xs font-bold text-slate-600">{task.model}</td>
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4 text-xs text-slate-400 font-medium">{task.date}</td>
                                <td className="px-6 py-4 text-right font-mono text-xs font-bold text-slate-900">{task.duration}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
    ...
    const renderAnalysis = () => (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-white/50 backdrop-blur-xl border border-slate-100 rounded-[32px] p-6 shadow-sm">
                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">头部模型得分对比 / SOTA Comparison</h3>
                <div className="h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={generalBenchmarks} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f3f4f6" />
                            <XAxis dataKey="name" tick={{ fontSize: 10, fontWeight: 900, fill: '#6b7280' }} />
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-end">
                <div className="text-left">
                    <h1 className="text-2xl font-black text-slate-900 tracking-tighter uppercase leading-none">模型评测工作室 Studio</h1>
                    <p className="mt-1 text-slate-500 font-medium text-xs">全维度、跨领域的大模型能力追踪与基准测试系统。</p>
                </div>
                <div className="flex bg-white/70 backdrop-blur-md p-1 rounded-2xl border border-slate-100 shadow-sm">
                    {[
